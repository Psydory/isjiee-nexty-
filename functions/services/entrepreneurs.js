...
// À ajouter dans entrepreneur.js
export async function incrementCompletedTasks(db, userId) {
  const now = Date.now();
  await db.prepare(`
    UPDATE entrepreneur_progression
...
export async function incrementCertificatesCount(db, userId) {
  const now = Date.now();
  await db.prepare(`
    UPDATE entrepreneur_progression
    SET certificates_count = certificates_count + 1, updated_at = ?
    WHERE user_id = ?
  `).bind(now, userId).run();
  await updateLeaderboard(db, userId);
}
